...
fn parse_desktop(path: &Path) -> Option<Install> {
    ...
    let channel = if path.file_name()?.to_str()?.contains("esr") {
        "esr"
    } else if path.file_name()?.to_str()?.contains("beta") {
        "beta"
    } else if path.file_name()?.to_str()?.contains("nightly") {
        "nightly"
    } else {
        "release"
    }
...
fn detect_default() -> Result<DefaultBrowser> {
        ...
        && out.status.success()
    {
        ...
        return Ok(DefaultBrowser {
            name: if is_ff {
                ...
                s.strip_suffix(".desktop").unwrap_or("Other").to_string()
            },
            is_ff_default: is_ff,
    ...
    for p in paths.into_iter().flatten() {
        if let Ok(text) = fs::read_to_string(p)
            && (text.contains("x-scheme-handler/http=firefox") || text.contains("=firefox.desktop"))
        {
            return Ok(DefaultBrowser {
                name: "Firefox".into(),
                is_ff_default: true,
                evidence: "mimeapps".into(),
            });
        }
    }
